 ...
 */
function checkHealth(serverUrl, authHeader) {
    return new Promise((resolve, reject) => {
...
function createApp(config) {
    ...
    // Build auth headers for OpenCode backend (Basic auth from env vars)
    const backendAuth = config.BACKEND_AUTH;
    const clientHeaders = {};
    if (backendAuth) {
        clientHeaders['Authorization'] = backendAuth;
    }
    ...
    try {
        ...
        if (isWindows) {
            ...
            if (useIsolatedHome) {
                ...
                // Pass auth credentials to backend if configured
                if (config.OPENCODE_SERVER_USERNAME) {
                    envVars.OPENCODE_SERVER_USERNAME = config.OPENCODE_SERVER_USERNAME;
                }
                if (config.OPENCODE_SERVER_PASSWORD) {
                    envVars.OPENCODE_SERVER_PASSWORD = config.OPENCODE_SERVER_PASSWORD;
                }
...
export function startProxy(options) {
    ...
    // Build backend auth header from env vars (OpenCode server uses Basic auth)
    const serverUsername = options.OPENCODE_SERVER_USERNAME || process.env.OPENCODE_SERVER_USERNAME || '';
    const serverPassword = options.OPENCODE_SERVER_PASSWORD || process.env.OPENCODE_SERVER_PASSWORD || '';
    let backendAuth = '';
    if (serverUsername && serverPassword) {
        const credentials = Buffer.from(`${serverUsername}:${serverPassword}`).toString('base64');
        backendAuth = `Basic ${credentials}`;
    }

    const config = {
        PORT: options.PORT || 8083,
        API_KEY: options.API_KEY || '',
        OPENCODE_SERVER_URL: options.OPENCODE_SERVER_URL || 'http://127.0.0.1:14097',
